...
To analyze how states navigate this geometry, historical time-series data (1990--2026) is projected onto the static reference manifold using a Nyström extension. This prevents historical volatility from warping the underlying geometric terrain.

We introduce two macro-po\-lit\-i\-cal metrics to quantify the institutional ``friction'' or ``wasted effort'' of a state's development over $N$ time steps:
\begin{enumerate}
    \item \textbf{Trajectory Tortuosity ($T$):} The ratio of the total Euclidean path length traversed over the net displacement \cite{benhamou2004reliably}. This measures how directly a state achieved its current governance configuration.
          $$T = \frac{\sum_{t=1}^{N-1} \lVert x_{t+1} - x_t \rVert}{\lVert x_N - x_1 \rVert}$$
    \item \textbf{Geodesic Inefficiency ($I$):} While Euclidean space ignores the manifold's shape, geodesic distance respects the ``terrain'' of allowable governance. We compute the shortest path $G$ through a $k$-NN graph of the manifold and define inefficiency as $I = \frac{G}{L}$, where $L$ is the Euclidean length. This measures the specific cost of instituting reforms that move \textit{against} the natural grain of the systemic landscape.
\end{enumerate}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{../figures/fig2_persistence_barcode.eps}
...
Projecting 5,628 historical observations (1990--2026) onto the manifold reveals that linear development is a statistical rarity. Applying unsupervised $K$-Means clustering ($K=3$) to the log-transformed Tortuosity and Geodesic Inefficiency metrics organically identifies three distinct movement paradigms:

\begin{itemize}
    \item \textbf{Direct ($N=88$):} States with low tortuosity and near-perfect geodesic efficiency, moving purposefully toward a stable attractor. The cluster medoid is the \textbf{Dominican Republic}.
    \item \textbf{Meandering ($N=67$):} States experiencing gradual, curving transitions through the manifold, adhering to natural geodesic paths but taking longer Euclidean routes. The cluster medoid is \textbf{Mauritania}.
    \item \textbf{Erratic ($N=15$):} States trapped in highly tortuous limit cycles. These states possess high Euclidean path lengths but immense Geodesic Inefficiency, indicating volatile institutional changes (e.g., repeating cycles of revolution and backsliding) that fight against the natural topology of the system. The cluster medoid is \textbf{Kyrgyzstan}.
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{../figures/fig3_trajectory_typologies.eps}
...
Analysis of systemic dispersion falsifies the post-Cold War convergence hypothesis. Figure 4 illustrates that while the global governance ensemble contracted significantly between 1990 and 1992 \cite{vicsek1995novel}, this convergence rapidly reversed. From the late 1990s through 2013, phase-space dispersion expanded relentlessly. Despite minor contractions during the 2020s, the overall trajectory indicates that the global system is actively polarizing into distant, competing topological basins.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../figures/fig4_global_dispersion.eps}
    \caption{\textbf{Phase-Space Dispersion Over Time.} The mean Euclidean distance of all states from the global centroid (1990--2026). The sharp rise in variance post-1992 highlights the polarization of the international system into distinct developmental attractors.}
